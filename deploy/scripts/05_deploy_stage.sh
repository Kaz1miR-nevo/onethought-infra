...
# =============================================================================
# Builds and deploys stage environment using Docker Compose
...
if [ ! -f "env/stage.api.env" ] || [ ! -f "env/stage.web.env" ]; then
    echo "❌ Error: Environment files not found!"
    echo "   Please create:"
    echo "   - env/stage.api.env (from env/stage.api.env.example)"
    echo "   - env/stage.web.env (from env/stage.web.env.example)"
    exit 1
fi

# Build and start containers
echo "🔨 Building and starting containers..."
docker compose -p onethought_stage -f docker-compose.stage.yml build --no-cache

echo "🚀 Starting containers..."
docker compose -p onethought_stage -f docker-compose.stage.yml up -d
...
echo ""
echo "📋 View logs: docker compose -p onethought_stage -f docker-compose.stage.yml logs -f"
